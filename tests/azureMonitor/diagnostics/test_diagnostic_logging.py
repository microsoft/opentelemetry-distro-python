...
def check_file_for_messages(file_path, level, messages):
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for message, message_id in messages:
            ...
            assert json["logger"] == "microsoft.azureMonitor._diagnostics.diagnostic_logging"
...
def set_up(
    ...
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging._DIAGNOSTIC_LOG_PATH",
        os.path.dirname(file_path),
    ).start()
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging._DIAGNOSTIC_LOGGER_FILE_NAME",
        os.path.basename(file_path),
    ).start()
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging._get_customer_ikey_from_env_var",
        return_value=TEST_CUSTOMER_IKEY,
    ).start()
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging._EXTENSION_VERSION",
        TEST_EXTENSION_VERSION,
    ).start()
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging.VERSION",
        TEST_VERSION,
    ).start()
    patch(
        "microsoft.azureMonitor._diagnostics.diagnostic_logging._is_diagnostics_enabled",
