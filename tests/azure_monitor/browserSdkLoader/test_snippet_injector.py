...
class TestWebSnippetInjector(unittest.TestCase):
    ...
    @patch("microsoft.opentelemetry._azure_monitor._browser_sdk_loader.snippet_injector._mark_browser_loader_feature")
    ...
    @patch("microsoft.opentelemetry._azure_monitor._browser_sdk_loader.snippet_injector.HAS_BROTLI", False)
    def test_decompress_content_brotli_not_available(self):
        """Test _decompress_content when brotli is not available."""
        content = b"some content"

        with patch(
            "microsoft.opentelemetry._azure_monitor._browser_sdk_loader.snippet_injector._logger"
        ) as mock_logger:
