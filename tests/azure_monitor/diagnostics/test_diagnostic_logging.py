...
import microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging as diagnostic_logger
...
def check_file_for_messages(file_path, level, messages):
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for message, message_id in messages:
            ...
            assert json["logger"] == "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging"
...
def set_up(
    ...
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._DIAGNOSTIC_LOG_PATH",
        os.path.dirname(file_path),
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._DIAGNOSTIC_LOGGER_FILE_NAME",
        os.path.basename(file_path),
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._get_customer_ikey_from_env_var",
        return_value=TEST_CUSTOMER_IKEY,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._EXTENSION_VERSION",
        TEST_EXTENSION_VERSION,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.VERSION",
        TEST_VERSION,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._is_diagnostics_enabled",
...
class TestDiagnosticLogger:
    ...
    def test_initialize_file_handler_exception(self, temp_file_path):
        ...
        with (
            patch(
                "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.logging.FileHandler"
            ) as mock_file_handler,
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._logger") as mock_logger,
        ):
    ...
    def test_initialize_makedirs_exception_not_file_exists(self, temp_file_path):
        ...
        with (
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.makedirs") as mock_makedirs,
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.exists", return_value=False),
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._logger") as mock_logger,
        ):
    ...
    def test_initialize_makedirs_file_exists_error_handled(self, temp_file_path):
        ...
        with (
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.makedirs") as mock_makedirs,
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.exists", return_value=False),
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._logger"),
        ):
    ...
    def test_initialize_formatter_exception(self, temp_file_path):
        ...
        with (
            patch(
                "microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging.logging.Formatter"
            ) as mock_formatter,
            patch("microsoft.opentelemetry._azure_monitor._diagnostics.diagnostic_logging._logger") as mock_logger,
        ):
