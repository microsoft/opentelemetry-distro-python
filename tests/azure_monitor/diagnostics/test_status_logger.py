...
def set_up(file_path, is_diagnostics_enabled=True):
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._STATUS_LOG_PATH",
        os.path.dirname(file_path),
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._get_status_logger_file_name",
        return_value=os.path.basename(file_path),
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._get_customer_ikey_from_env_var",
        return_value=TEST_CUSTOMER_IKEY,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._EXTENSION_VERSION",
        TEST_EXTENSION_VERSION,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger.VERSION",
        TEST_VERSION,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._is_diagnostics_enabled",
        return_value=is_diagnostics_enabled,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger.getpid",
        return_value=TEST_PID,
    ).start()
    patch(
        "microsoft.opentelemetry._azure_monitor._diagnostics.status_logger._MACHINE_NAME",
        TEST_MACHINE_NAME,
    ).start()
