...
from microsoft.opentelemetry._instrumentation import (
    DependencyConflict,
...
class TestDependencyConflict(unittest.TestCase):
    ...
    def test_str_default(self):
        c = DependencyConflict(required="pkg>=1.0")
        self.assertIn("pkg>=1.0", str(c))
...
class TestGetDistDependencyConflicts(unittest.TestCase):
    ...
    def test_instruments_dep_satisfied(self):
        dist = MagicMock()
        dist.requires = ['pytest>=8.0; extra == "instruments"']
        result = get_dist_dependency_conflicts(dist)
        self.assertIsNone(result)

    def test_instruments_any_dep_satisfied(self):
        dist = MagicMock()
        dist.requires = ['pytest>=8.0; extra == "instruments-any"']
        result = get_dist_dependency_conflicts(dist)
        self.assertIsNone(result)

    def test_no_extra_marker(self):
        dist = MagicMock()
        dist.requires = ["pytest>=8.0"]
        self.assertIsNone(get_dist_dependency_conflicts(dist))
...
class TestGetDependencyConflicts(unittest.TestCase):
    ...
    def test_string_dep_not_found(self):
        result = get_dependency_conflicts(["nonexistent-xyz-package>=1.0"])
    ...
    def test_with_deps_any_satisfied(self):
